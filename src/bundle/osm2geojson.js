...
export const geojson2osm = function (geo, changeset, osmChange) {
  function togeojson(geo, properties) {
    ...
    switch (geo.type) {
      ...
      case 'MultiPolygon':
        relations +=
          '<relation id="' + count + '" changeset="' + changeset + '">';
    ...
    osm =
      '<?xml version="1.0" encoding="UTF-8"?><osm version="0.6" generator="github.com/aaronlidman/osm-and-geojson">' +
      nodes +
      ways +
      relations +
      '</osm>';
    if (osmChange) {
      osm =
        '<osmChange version="0.6" generator="github.com/aaronlidman/osm-and-geojson"><create>' +
        nodes +
        ways +
        relations +
        '</create></osmChange>';
  ...
  function polygon(geo, properties, multipolygon) {
    ...
    if (geo.coordinates.length > 1) {
      ...
      for (var i = 0; i < geo.coordinates.length; i++) {
        ...
        relations +=
          '<member type="way" ref="' + count + '" role="' + role + '"/>';
  ...
  switch (geo.type) {
    case 'FeatureCollection':
      ...
      for (var i = 0; i < geo.features.length; i++) {
        obj.push(
          togeojson(geo.features[i].geometry, geo.features[i].properties),
        );
      }
      temp.osm =
        '<?xml version="1.0" encoding="UTF-8"?><osm version="0.6" generator="github.com/aaronlidman/osm-and-geojson">';
    ...
    default:
      if (console)
        console.error(
          'Invalid GeoJSON object: GeoJSON object must be one of "Point", "LineString", ' +
  ...
  function Points(nodeCache) {
    ...
    for (var node in nodeCache) {
      ...
      if (!usedCoords[node] || tags.length)
        features.push(
          getFeature(nodeCache[node], 'Point', lonLat(nodeCache[node])),
        );
    }
          ...
          if (feature.geometry.coordinates.length > 1) {
            ...
            for (var a = 0; a < feature.geometry.coordinates.length; a++) {
              if (
                ...
                feature.geometry.coordinates[a].push(
                  way.geometry.coordinates[0],
                );
                break;
              }
            }
